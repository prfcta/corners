...
def convert_from_json_to_nparray(*args):
    """
    Функция конвертирует json в numpy.
    """
    hc, ac, hg, ag = [np.array(json.loads(minute)) for minute in args]
    return hc, ac, hg, ag


def amount_of_corners_after_x_minute(hc, ac, hg, ag, minute, term):
...
def work(result, analysis_answer, minute):
    ...
    for match in result:
        ...
        if analysis_answer == 1:
            ...
            term = 'draw'
            answer = amount_of_corners_after_x_minute(hc, ac, hg, ag, minute, term)
